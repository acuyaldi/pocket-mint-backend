...
import type { RuleCandidate, RuleMatch, RuleMatchInput, RuleOperator } from './types';

function compare(haystack: string, needle: string, operator: RuleOperator): boolean {
...
function matchesRule(rule: RuleCandidate, input: RuleMatchInput): boolean {
  ...
  switch (rule.matchType) {
    ...
    case 'MERCHANT':
      return compare(normalizeMerchant(input.description), normalizeMerchant(rule.value), rule.operator);
 ...
 * generateSuggestions().
 */
export function matchRules(rules: RuleCandidate[], input: RuleMatchInput): RuleMatch | null {
  const trimmedDescription = input.description.trim();
  if (trimmedDescription.length === 0) return null;
