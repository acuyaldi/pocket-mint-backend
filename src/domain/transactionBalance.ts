 ...
 */
function balanceDeltas(input: BalanceEffectInput, strict: boolean): WalletDelta[] {
  ...
  if (input.isInstallment) {
    ...
    if (grand === null) {
      if (strict) throw new Error('installment balance effect requires installmentGrandTotal');
  ...
  switch (type) {
    ...
    case 'EXPENSE':
      return [{ walletId, amount: input.amount.negated() }];
    case 'TRANSFER': {
      const deltas: WalletDelta[] = [{ walletId, amount: input.amount.negated() }];
      if (input.toWalletId) {
...
export function computeBalanceEffect(input: BalanceEffectInput): WalletDelta[] {
  return balanceDeltas(input, true);
}

/** Deltas to undo a persisted transaction's effect — the exact negation of create. */
export function reverseBalanceEffect(input: BalanceEffectInput): WalletDelta[] {
...
export async function applyBalanceDeltas(
  txClient: WalletBalanceClient,
  deltas: WalletDelta[]
): Promise<void> {
  for (const delta of deltas) {
    if (delta.amount.isZero()) continue;
...
// ------------------------------------------------------------
// Reconciliation (read-only, for tests and the audit script)
// ------------------------------------------------------------
...
export interface WalletReconciliation {
  ...
  drift: Prisma.Decimal; // stored - expected; zero when consistent
}
...
export function reconcileWalletBalances(
  ...
  for (const w of wallets) expected.set(w.id, w.initialBalance);

  const add = (walletId: string, amount: Prisma.Decimal) => {
    if (!expected.has(walletId)) return; // ignore effects on unknown wallets
  ...
  for (const tx of transactions) {
    ...
    for (const d of deltas) add(d.walletId, d.amount);
  }
