...
export class ToolRegistry {
  ...
  register(tool: ToolContract): void {
    if (this.tools.has(tool.id)) {
      throw AssistantError.duplicateRegistration(tool.id);
    }
  ...
  private validateInvariants(tool: ToolContract): void {
    if (tool.timeoutMs <= 0) {
      throw AssistantError.invalidTimeout(tool.id, tool.timeoutMs);
    }
    ...
    if (
      ...
      throw AssistantError.policyMismatch(
        tool.id,
        `${tool.riskLevel} risk requires at least EXPLICIT confirmation`,
      );
    }
    ...
    if (tool.riskLevel === 'VERY_HIGH' && tool.confirmationPolicy !== 'DISABLED') {
      throw AssistantError.policyMismatch(
        tool.id,
        'VERY_HIGH risk requires DISABLED confirmation policy',
      );
    }
  }
}
