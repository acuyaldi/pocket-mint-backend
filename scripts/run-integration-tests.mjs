...
async function main() {
  ...
  if (existing) {
    ...
    run('npx', ['vitest', 'run', 'test/prismaAdapter.integration.test.ts'], { TEST_DATABASE_URL: existing });
    return;
  }
  ...
  try {
    ...
    run('npx', ['vitest', 'run', 'test/prismaAdapter.integration.test.ts'], { TEST_DATABASE_URL: testDatabaseUrl });
  } finally {
