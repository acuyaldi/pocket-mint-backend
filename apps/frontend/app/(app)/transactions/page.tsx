...
export default function TransactionsPage() {
  ...
  const handleAddSubmit = useCallback(async (d: { description: string; amount: number; type: "EXPENSE" | "INCOME"; date: string; walletId?: string }) => {
    setIsCreating(true); try { await createTransaction.mutateAsync(d); setIsAddModalOpen(false); } catch (e) { console.error(e); } finally { setIsCreating(false); }
  }, [createTransaction]);

  return (
    <div className="space-y-6">
      <motion.main className="space-y-6" initial="hidden" animate="visible" variants={{ visible: { transition: { staggerChildren: 0.08 } } }}>
        <TransactionHeader onAddClick={() => setIsAddModalOpen(true)} />
